...
class NLPService {
  ...
  async detectIntent(text: string): Promise<{
    ...
    const intentPatterns = [
      ...
      { pattern: /\b(restaurant|restaurants|dining|where to eat|best food|eat at|dine)\b/, intent: 'restaurant_query', confidence: 0.9 },
      { pattern: /\b(food|cuisine|dish|famous food|must try|local food|delicacy|sweets|snacks)\b/, intent: 'food_query', confidence: 0.88 },
      ...
      { pattern: /\b(hotel|hotels|stay|accommodation|resort|resorts|where to stay|lodging|book room|oyo|taj|oberoi|marriott)\b/, intent: 'hotel_query', confidence: 0.9 },
      ...
      { pattern: /\b(district|districts|cities in|towns in|taluks)\b/, intent: 'district_query', confidence: 0.88 },
